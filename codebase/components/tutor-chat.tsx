...
import type { FocusIncident } from "@/components/focus-tracker"
...
export function TutorChat({
    ...
    focusIncident?: FocusIncident | null
    onCollapse: () => void
}) {
    ...
    return (
        <section aria-label="AI Tutor" className="flex min-h-0 flex-1 flex-col rounded-xl border border-border bg-card shadow-sm">
            ...
            <div className="flex flex-1 flex-col gap-5 overflow-y-auto px-4 py-4">
                {focusIncident && dismissedFocusId !== focusIncident.id ? (
                    <div className="rounded-xl border border-amber-300/70 bg-amber-50 px-3 py-3 dark:border-amber-700/60 dark:bg-amber-950/30">
                        <div className="flex items-start gap-2">
                            <Sparkles className="mt-0.5 size-4 shrink-0 text-amber-600" aria-hidden="true" />
                            <div className="min-w-0 flex-1">
                                <p className="text-sm font-medium text-amber-950 dark:text-amber-100">
                                    Bạn có vẻ đang mất tập trung, cần nghỉ 1 chút không?
                                </p>
                                <div className="mt-2 flex gap-2">
                                    <button
                                        type="button"
                                        onClick={() => setDismissedFocusId(focusIncident.id)}
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setDismissedFocusId(focusIncident.id)}
                                        className="rounded-md px-2.5 py-1.5 text-xs text-amber-800 hover:bg-amber-100 dark:text-amber-200 dark:hover:bg-amber-900/50"
                                    >
                                        Bỏ qua
                                    </button>
                                </div>
                                    ...
                                    <SuggestionBlock
                                        suggestions={
                                            turn.suggestions?.length
                                                ? turn.suggestions
                                                : idleSuggestions.map((q, i) => ({ id: `idle-${i}`, question: q, page: currentPage ?? 1, tag: "Ôn nhanh" as const }))
                                        }
