...
function buildSystemPrompt(deck: AskBody["deck"], currentPage: number | null, rag: { slideContext: string; transcriptContext: string; citationPages: number[] }): string {
  ...
  const citationNote =
    rag.citationPages.length > 0
      ? `Trang được trích dẫn: ${rag.citationPages.join(", ")}.`
      : ""
  ...
  return [
    ...
    "- Nếu có citations, kết thúc câu trả lời bằng 1 câu hỏi gợi ý đào sâu thêm (phủ định / so sánh / ví dụ).",
    deckContext,
    pageNote,
    citationNote,
  ]
...
async function callGemini(
...
): Promise<{ content: string; page: number | null }> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Gemini ${res.status}: ${errText.slice(0, 200) || res.statusText}`)
  }
  ...
  return { content: text, page: extractPageFromText(text) }
}
...
async function callMistral(
  ...
  if (!text) throw new Error("Mistral trả về phản hồi rỗng.")
  return { content: text, page: extractPageFromText(text) }
}
...
async function callOpenRouter(
  ...
  const data = (await res.json()) as { choices?: { message?: { content?: string } }[] }
  const text = data.choices?.[0]?.message?.content?.trim() ?? ""
  if (!text) throw new Error("OpenRouter trả về phản hồi rỗng.")
  return { content: text, page: extractPageFromText(text) }
}
...
export async function POST(req: Request) {
  ...
  try {
    const result =
      provider === "mistral"
        ? await callMistral(apiKey, systemInstruction, question, body.history)
        : provider === "openrouter"
          ? await callOpenRouter(apiKey, systemInstruction, question, body.history)
          : await callGemini(apiKey, systemInstruction, question, body.history)

    // Use RAG citation pages as primary; fall back to page mentioned in LLM text.
    const citationPages = rag.citationPages.length > 0 ? rag.citationPages : (result.page ? [result.page] : [])

    return NextResponse.json({
      content: result.content,
      page: result.page ?? (citationPages.length > 0 ? citationPages[0] : null),
      provider,
      citations: citationPages,
      ragConfigured: ragForIntegrity.qdrantConfigured,
    })
  } catch (err) {
    console.error(`[tutor] ${provider} failed:`, err)
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Không gọi được LLM" },
      { status: 502 },
    )
  }
}
